...
const depositMoney = async (rulesDeposit, id) => {
  ...
  moneyId.balance = moneyId.balance + value;

  const accountupdate = await Account.findByIdAndUpdate(id, {
    balance: moneyId.balance,
  });
...
const withdrawMoney = async (ruleWithdraw, id) => {
  ...
  withdrawId.balance = withdrawId.balance - value;

  const accountupdate = await Account.findByIdAndUpdate(id, {
    balance: withdrawId.balance,
  });
...
const checkStatement = async (id) => {
  ...
  // testar depois que criar models transaction
